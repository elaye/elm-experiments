...
circleColor radius =
  let r = round <| 255 * (toFloat radius) / 70 + 30
      g = round <| 255 * (toFloat radius) / 40 + 10
      b = round <| 255 * (toFloat radius) / 70 + 25
  in rgba r g b 1
...
mouseToPos: Int -> Signal (Int, Int)
mouseToPos r = 
  let nx = map2 (\x c -> (toFloat x) / w - 0.5) Mouse.x Window.width
      ny = map2 (/) Mouse.y Window.height
